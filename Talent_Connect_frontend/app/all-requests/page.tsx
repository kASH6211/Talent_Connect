...
type TabId = "requests" | "offers" | "accepted";
...
export default function AllRequestsPage() {
  ...
  const filteredRequests = requests.filter((r) => {
    ...
    const matchStatus =
      reqStatus === "All" ||
      status.toLowerCase() === reqStatus.toLowerCase();
  ...
  const filteredOffers = offers.filter((o) => {
    ...
    const matchStatus =
      offStatus === "All" ||
      status.toLowerCase() === offStatus.toLowerCase();
    ...
    if (offDateStart || offDateEnd) {
      ...
      if (offerDate && filterStart && offerDate < filterStart) matchDate = false;
    ...
    if (lastDateStart || lastDateEnd) {
      ...
      const filterLStart = lastDateStart ? new Date(lastDateStart).getTime() : 0;
      const filterLEnd = lastDateEnd ? new Date(lastDateEnd).getTime() : Infinity;

      if (lastDate && filterLStart && lastDate < filterLStart) matchLastDate = false;
      if (lastDate && filterLEnd && lastDate > filterLEnd) matchLastDate = false;
    }
  ...
  const acceptedOffers = offers.filter((o) => {
    ...
    return name.toLowerCase().includes(searchLow) || title.toLowerCase().includes(searchLow);
  });
  ...
  const exportToExcel = () => {
    ...
    const data = filteredOffers.map(o => ({
      ...
      "Offer Date": o.offer_date ? new Date(o.offer_date).toLocaleDateString("en-IN") : "—",
      "Last Date": o.last_date ? new Date(o.last_date).toLocaleDateString("en-IN") : "—",
      "Status": o.status || "—"
    }));
  ...
  const exportRequestsToExcel = () => {
    ...
    const data = filteredRequests.map(r => ({
      ...
      "Request Type": r.requestType?.request_type_name || r.job_title || "General",
      "Request Date": r.request_date ? new Date(r.request_date).toLocaleDateString("en-IN") : "—",
      "Expiry Date": r.expiry_date ? new Date(r.expiry_date).toLocaleDateString("en-IN") : "—",
    ...
    <div className="w-full mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-xl bg-primary flex items-center justify-center shadow-lg shadow-primary/30">
            <TrendingUp size={22} className="text-primary-content" strokeWidth={2} />
          </div>
          <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {stats.map((s) => (
          ...
          >
            ...
            <p className="text-xs font-semibold text-base-content/60 leading-tight">{s.label}</p>
          </div>
        ))}
      </div>
      ...
      {activeTab === "requests" && (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl bg-base-100 dark:bg-base-900 border border-base-300 dark:border-base-700 shadow-sm p-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-base-content">Filters & Export</h2>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              <div>
                ...
                <select
                  value={reqStatus}
                  onChange={(e) => { setReqStatus(e.target.value); reqPag.setPage(1); }}
              ...
              <div className="sm:col-span-3">
                ...
                <div className="relative">
                  <input
                    ...
                    value={reqSearch}
                    onChange={(e) => { setReqSearch(e.target.value); reqPag.setPage(1); }}
                  ...
                  {reqSearch && (
                    <button onClick={() => setReqSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-base-content/40 hover:text-error">
                      <X size={14} />
                    </button>
                  )}
                </div>
            ...
            {(reqSearch || reqStatus !== "All") && (
              <button
                onClick={() => { setReqSearch(""); setReqStatus("All"); reqPag.setPage(1); }}
          ...
          {loadingReq ? (
            ...
            <EmptyState icon={<Briefcase size={24} className="text-primary" />} title="No requests found" sub="No placement requests match your filters" />
          ) : (
            <>
                ...
                <table className="min-w-full divide-y divide-base-200 dark:divide-base-800">
                  ...
                  <tbody className="divide-y divide-base-200 dark:divide-base-800">
                    {reqPag.slice.map((r) => {
                      ...
                      const type = r.requestType?.request_type_name ?? r.job_title ?? "General";
                      const status = r.requestStatus?.status_name ?? r.status ?? "Pending";
                      return (
                        <tr key={r.industry_request_id ?? r.request_id} className="hover:bg-base-200/40 dark:hover:bg-base-800/40 transition-colors">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <Building2 size={15} className="text-primary/70 flex-shrink-0" />
                              <span className="text-sm font-medium text-base-content">{name}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3 text-sm text-base-content/70">{dist}</td>
                          <td className="px-4 py-3 text-sm text-base-content/70">{type}</td>
                          <td className="px-4 py-3 text-sm text-base-content/70">{fmt(r.request_date)}</td>
                          <td className="px-4 py-3 text-sm text-base-content/70">{r.vacancies ?? "—"}</td>
              ...
              <PaginationBar page={reqPag.page} totalPages={reqPag.totalPages} total={reqPag.total} onPage={reqPag.setPage} label="requests" />
            </>
          )}
        </div>
      )}
      ...
      {activeTab === "offers" && (
        <div className="space-y-4">
          <div className="rounded-2xl bg-base-100 dark:bg-base-900 border border-base-300 dark:border-base-700 shadow-sm p-5">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-base-content/60 mb-1.5 uppercase tracking-wide">Status</label>
                  <select
                    value={offStatus}
                    onChange={(e) => { setOffStatus(e.target.value); offPag.setPage(1); }}
                ...
                <div className="sm:col-span-1 md:col-span-2">
                  <label className="block text-xs font-semibold text-base-content/60 mb-1.5 uppercase tracking-wide">Search</label>
                  <div className="relative">
                    <input
                      type="text"
                      placeholder="Search institute, job title…"
                      value={offSearch}
                      onChange={(e) => { setOffSearch(e.target.value); offPag.setPage(1); }}
                    ...
                    <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-base-content/40 pointer-events-none" />
                    {offSearch && (
                      <button onClick={() => setOffSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-base-content/40 hover:text-error">
                        <X size={14} />
                      </button>
                    )}
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-base-content/60 mb-1.5 uppercase tracking-wide">Offer Date (From)</label>
                  <input
                    type="date"
                    value={offDateStart}
                    onChange={(e) => { setOffDateStart(e.target.value); offPag.setPage(1); }}
                    className="w-full bg-base-200 dark:bg-base-800 border border-base-300 dark:border-base-700 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-info"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-base-content/60 mb-1.5 uppercase tracking-wide">Offer Date (To)</label>
                  <input
                    type="date"
                    value={offDateEnd}
                    onChange={(e) => { setOffDateEnd(e.target.value); offPag.setPage(1); }}
                    className="w-full bg-base-200 dark:bg-base-800 border border-base-300 dark:border-base-700 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-info"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-base-content/60 mb-1.5 uppercase tracking-wide">Last Date (From)</label>
                  <input
                    type="date"
                    value={lastDateStart}
                    onChange={(e) => { setLastDateStart(e.target.value); offPag.setPage(1); }}
                    className="w-full bg-base-200 dark:bg-base-800 border border-base-300 dark:border-base-700 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-info"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-base-content/60 mb-1.5 uppercase tracking-wide">Last Date (To)</label>
                  <input
                    type="date"
                    value={lastDateEnd}
                    onChange={(e) => { setLastDateEnd(e.target.value); offPag.setPage(1); }}
                    className="w-full bg-base-200 dark:bg-base-800 border border-base-300 dark:border-base-700 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-info"
                  />
                </div>
              </div>
            ...
            {(offSearch || offStatus !== "All" || offDateStart || offDateEnd || lastDateStart || lastDateEnd) && (
              <div className="mt-4 flex justify-end">
                <button
                  onClick={() => { setOffSearch(""); setOffStatus("All"); setOffDateStart(""); setOffDateEnd(""); setLastDateStart(""); setLastDateEnd(""); offPag.setPage(1); }}
          ...
          {loadingOff ? (
            ...
            <EmptyState icon={<Send size={24} className="text-info" />} title="No offers found" sub="No industry offers match your filters" />
          ) : (
            <>
              <div className="text-sm text-base-content/60 px-1">Showing {offPag.slice.length} of {offPag.total} offers</div>
              <div className="overflow-x-auto rounded-xl border border-base-300 dark:border-base-700 bg-base-100 dark:bg-base-900 shadow-sm">
                <table className="min-w-full divide-y divide-base-200 dark:divide-base-800">
                  <thead>
                    <tr className="bg-base-200/60 dark:bg-base-800/60">
                  ...
                  <tbody className="divide-y divide-base-200 dark:divide-base-800">
                    {offPag.slice.map((o) => (
                      <tr key={o.offer_id} className="hover:bg-base-200/40 dark:hover:bg-base-800/40 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <Building2 size={15} className="text-info/70 flex-shrink-0" />
                            <div>
                              <p className="text-sm font-medium text-base-content">{o.institute?.institute_name ?? "—"}</p>
                              {getDistrictName(o) !== "—" && <p className="text-xs text-base-content/50">{getDistrictName(o)}</p>}
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <p className="text-sm font-medium text-base-content max-w-[180px] truncate">{o.job_title}</p>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 text-sm text-base-content/70">{fmt(o.offer_date)}</td>
                        <td className="px-4 py-3 text-sm text-base-content/70">
                          <div className="flex items-center gap-1">
                            <CalendarDays size={13} className="flex-shrink-0 text-warning/70" />
                            {fmt(o.last_date)}
                          </div>
                        </td>
              ...
              <PaginationBar page={offPag.page} totalPages={offPag.totalPages} total={offPag.total} onPage={offPag.setPage} label="offers" />
            </>
          )}
        </div>
      )}
      ...
      {activeTab === "accepted" && (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl bg-base-100 dark:bg-base-900 border border-base-300 dark:border-base-700 shadow-sm p-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-base-content">Search & Export</h2>
              </div>
              <button
            ...
            <div className="relative">
              <input
                ...
                value={accSearch}
                onChange={(e) => { setAccSearch(e.target.value); accPag.setPage(1); }}
              ...
              <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-base-content/40 pointer-events-none" />
              {accSearch && (
                <button onClick={() => setAccSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-base-content/40 hover:text-error">
                  <X size={14} />
                </button>
              )}
            </div>
          ...
          {loadingOff ? (
            ...
            <EmptyState icon={<CheckCircle2 size={24} className="text-success" />} title="No accepted offers" sub="No accepted offers found yet" />
          ) : (
            <>
              <div className="text-sm text-base-content/60 px-1">Showing {accPag.slice.length} of {accPag.total} accepted offers</div>
              <div className="overflow-x-auto rounded-xl border border-base-300 dark:border-base-700 bg-base-100 dark:bg-base-900 shadow-sm">
                <table className="min-w-full divide-y divide-base-200 dark:divide-base-800">
                  <thead>
                  ...
                  <tbody className="divide-y divide-base-200 dark:divide-base-800">
                    {accPag.slice.map((o) => (
                      <tr key={o.offer_id} className="hover:bg-success/5 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                        ...
                        <td className="px-4 py-3">
                          <p className="text-sm font-semibold text-base-content">{o.job_title}</p>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 text-sm text-base-content/70">{fmt(o.offer_date)}</td>
                        <td className="px-4 py-3 text-sm text-base-content/70">{fmt(o.last_date)}</td>
                        <td className="px-4 py-3 text-right">
                          <button
                            onClick={() => setViewOffer(o)}
              ...
              <PaginationBar page={accPag.page} totalPages={accPag.totalPages} total={accPag.total} onPage={accPag.setPage} label="accepted offers" />
            </>
          )}
        </div>
      )}
      ...
      {viewOffer && (
        <Modal title="Offer Details" onClose={() => setViewOffer(null)}>
          ...
          <DetailRow label="District" value={viewOffer.institute?.district ?? "—"} />
          <DetailRow label="Job Title" value={viewOffer.job_title} />
          {viewOffer.job_description && <DetailRow label="Description" value={viewOffer.job_description} />}
          <DetailRow label="Salary Range" value={salary(viewOffer.salary_min, viewOffer.salary_max)} />
          <DetailRow label="Number of Posts" value={viewOffer.number_of_posts?.toString() ?? "—"} />
      ...
      {viewReq && (
        <Modal title="Request Details" onClose={() => setViewReq(null)}>
          <DetailRow label="Institute" value={viewReq.institute?.institute_name ?? viewReq.institute_name ?? "—"} />
          <DetailRow label="District" value={viewReq.institute?.district ?? viewReq.district ?? "—"} />
          <DetailRow label="Request Type" value={viewReq.requestType?.request_type_name ?? viewReq.job_title ?? "—"} />
          <DetailRow label="Request Date" value={fmt(viewReq.request_date)} />
          <DetailRow label="Expiry Date" value={fmt(viewReq.expiry_date)} />
          <DetailRow label="Vacancies" value={viewReq.vacancies?.toString() ?? "—"} />
          {viewReq.remarks && <DetailRow label="Remarks" value={viewReq.remarks} />}
          <div>
            <p className="text-xs text-base-content/60 mb-1">Status</p>
            <StatusBadge status={viewReq.requestStatus?.status_name ?? viewReq.status ?? "Pending"} />
          </div>
        </Modal>
      )}
    </div>
...
function EmptyState({ icon, title, sub }: { icon: React.ReactNode; title: string; sub: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 rounded-2xl border-2 border-dashed border-base-300 dark:border-base-700 bg-base-100 dark:bg-base-900 gap-3">
      <div className="w-14 h-14 rounded-2xl bg-base-200 dark:bg-base-800 flex items-center justify-center">{icon}</div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-base-100 dark:bg-base-900 rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto border border-base-300 dark:border-base-700 shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-base-200 dark:bg-base-800 flex items-center justify-center hover:bg-error/10 hover:text-error transition-colors">
            <X size={18} />
          </button>
        </div>
