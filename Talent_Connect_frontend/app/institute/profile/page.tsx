...
function useOptions(apiPath: string, valueKey: string, labelKey: string) {
  ...
  useEffect(() => {
    api
      ...
      .then((r) =>
        setOpts(
          r.data.map((x: any) => ({ value: x[valueKey], label: x[labelKey] })),
        ),
      )
      .catch(() => {});
  }, [apiPath]);
...
const Section = ({
  ...
  <div className="bg-base-100 border border-base-200 dark:border-base-700 rounded-2xl p-6 space-y-4 shadow-sm">
    <p className="text-xs font-bold uppercase tracking-widest text-base-content/50 border-b border-base-200 dark:border-base-700 pb-2">
      {title}
    </p>
...
export default function InstituteProfilePage() {
  ...
  // All dropdowns
  const typeOpts = useOptions(
    "institute-type",
    "institute_type_id",
    "institute_type",
  );
  const subTypeOpts = useOptions(
    "institute-sub-type",
    "institute_sub_type_id",
    "institute_sub_type",
  );
  const ownershipOpts = useOptions(
    "institute-ownership-type",
    "institute_ownership_type_id",
    "institute_type",
  );
  const affiliationOpts = useOptions(
    "affiliation",
    "affiliating_body_id",
    "affiliating_body",
  );
  const regulatoryOpts = useOptions(
    "regulatory",
    "regulatory_body_id",
    "regulatory_body",
  );
  const boardOpts = useOptions(
    "board",
    "university_board_id",
  ...
  const enrollmentOpts = useOptions(
    "institute-enrollment",
    "institute_enrollment_id",
    "instituteenrollmenttype",
  );
  const trainingTypeOpts = useOptions(
    "training-type",
    "training_type_id",
    "training_type",
  );
  ...
  useEffect(() => {
    ...
    api
      .get(`/district?state_id=${data.lgdstateId}`)
      .then((r) =>
        setDistrictOpts(
          r.data.map((d: any) => ({
            value: d.districtid,
    ...
    <div className="max-w-4xl  mx-auto space-y-6 pb-10">
      ...
      <Section title="Basic Information">
        <div className="md:col-span-2">
          <FIELD label="Institute Name" required>
            <INPUT
              ...
              onChange={setInput("institute_name")}
            />
          </FIELD>
        </div>
        <FIELD label="Abbreviation">
          <INPUT
            value={data.institute_abbreviation || ""}
            onChange={setInput("institute_abbreviation")}
          />
        </FIELD>
        <FIELD label="Registration ID">
          <INPUT
            value={data.institute_registration_id || ""}
            disabled
        ...
        <FIELD label="Year of Establishment">
          <INPUT
            ...
            onChange={setInput("year_of_establishment")}
          />
        </FIELD>
      ...
      <Section title="Classification">
        <FIELD label="Institute Type" required>
          <SELECT
            value={data.institute_type_id}
            onChange={set("institute_type_id")}
            opts={typeOpts}
          />
        </FIELD>
        <FIELD label="Institute Sub Type">
          <SELECT
            value={data.institute_sub_type_id}
            onChange={set("institute_sub_type_id")}
            opts={subTypeOpts}
          />
        </FIELD>
        <FIELD label="Ownership Type">
          <SELECT
            value={data.institute_ownership_type_id}
            onChange={set("institute_ownership_type_id")}
            opts={ownershipOpts}
          />
        </FIELD>
        <FIELD label="Affiliating Body">
          <SELECT
            value={data.affiliating_body_id}
            onChange={set("affiliating_body_id")}
            opts={affiliationOpts}
          />
        </FIELD>
        <FIELD label="Regulatory Body">
          <SELECT
            value={data.regulatory_body_id}
            onChange={set("regulatory_body_id")}
            opts={regulatoryOpts}
          />
        </FIELD>
        <FIELD label="University / Board">
          <SELECT
            value={data.university_board_id}
            onChange={set("university_board_id")}
            opts={boardOpts}
          />
        </FIELD>
        <FIELD label="Enrollment Type">
          <SELECT
            value={data.institute_enrollment_id}
            onChange={set("institute_enrollment_id")}
            opts={enrollmentOpts}
          />
        </FIELD>
        <FIELD label="Rural / Urban">
          <select
            value={data.institute_rural_urban_status || ""}
            onChange={(e) =>
              set("institute_rural_urban_status")(e.target.value)
            }
            className="select select-bordered w-full text-sm"
          >
        ...
        <FIELD label="Total Seat Intake">
          <INPUT
            type="number"
            value={data.totalseatIntake || ""}
            onChange={setInput("totalseatIntake")}
          />
        </FIELD>
      </Section>
      ...
      <Section title="Contact Details">
        ...
        <FIELD label="Contact Person">
          <INPUT
            ...
            onChange={setInput("contactperson")}
          />
        </FIELD>
        <FIELD label="Designation">
          <INPUT
            value={data.designation || ""}
            onChange={setInput("designation")}
          />
        </FIELD>
      </Section>
      ...
      <Section title="Location">
        <FIELD label="State">
          <select
            value={data.lgdstateId || ""}
            onChange={(e) => {
              set("lgdstateId")(e.target.value ? +e.target.value : null);
              set("lgddistrictId")(null);
            }}
            className="select select-bordered w-full text-sm"
          >
        ...
        <FIELD label="District">
          <SELECT
            value={data.lgddistrictId}
            onChange={set("lgddistrictId")}
            opts={districtOpts}
      ...
      <Section title="Placement & Training">
        <FIELD label="Placement Cell Available?">
          <RADIO
            value={data.is_placement_cell_available || "N"}
            onChange={set("is_placement_cell_available")}
            label=""
          />
        </FIELD>
        <FIELD label="Training Offered?">
          <RADIO
            value={data.is_institute_offerened_training || "N"}
            onChange={set("is_institute_offerened_training")}
            label=""
          />
        </FIELD>
        <FIELD label="Placement Officer Mobile">
          <INPUT
            value={data.placement_officer_contact_number || ""}
            onChange={setInput("placement_officer_contact_number")}
          />
        </FIELD>
        <FIELD label="Placement Officer Email">
          <INPUT
            type="email"
            value={data.placement_officer_email_id || ""}
            onChange={setInput("placement_officer_email_id")}
          />
        </FIELD>
      </Section>
    </div>
