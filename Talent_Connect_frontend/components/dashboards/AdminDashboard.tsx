...
import { Building2, Factory, Users, Briefcase, GraduationCap } from 'lucide-react';
...
export default function AdminDashboard({ username }: { username: string }) {
    ...
    const statCards = [
        { label: 'Institutes', count: counts.institutes, icon: Building2, color: 'from-primary to-primary/70', href: '/institutes' },
        { label: 'Industries', count: counts.industries, icon: Factory, color: 'from-secondary to-secondary/70', href: '/industries' },
        { label: 'Students', count: counts.students, icon: Users, color: 'from-success to-success/70', href: '/students' },
        { label: 'Placements', count: counts.placements, icon: Briefcase, color: 'from-warning to-warning/70', href: '/placements' },
    ];
        ...
        <div className="space-y-8">
            ...
            <div className="relative overflow-hidden p-7 rounded-2xl bg-gradient-to-br from-primary/10 via-base-200 to-base-200 border border-primary/20">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-base-content">Welcome back, {username} 👋</h1>
                        <p className="text-primary/80 text-sm mt-0.5">Administrator · Talent Connect Portal</p>
                    </div>
                </div>
                <p className="mt-4 text-base-content/60 text-sm leading-relaxed max-w-2xl">
                    Full access to all modules — manage institutes, industries, students, placements, and master data from here.
                </p>
            </div>

            {/* Quick Access */}
            <div>
                <h2 className="text-xs font-semibold text-base-content/50 uppercase tracking-widest mb-4">Quick Access</h2>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {statCards.map(s => (
                        <Link key={s.label} href={s.href}
                            ...
                            <div className="text-base font-bold text-base-content">{s.count}</div>
            ...
            <div>
                <h2 className="text-xs font-semibold text-base-content/50 uppercase tracking-widest mb-4">Master Data</h2>
            ...
            {/* Mappings */}
            <div>
                <h2 className="text-xs font-semibold text-base-content/50 uppercase tracking-widest mb-4">Mappings</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {[
                        ...
                        <Link key={l.href} href={l.href}
                            ...
                            {l.label}
                            <span className="text-base-content/30 group-hover:text-secondary transition-colors">→</span>
                        </Link>
                    ))}
                </div>
            </div>
